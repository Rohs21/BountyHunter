...
const NewBountyBody = () => {
  ...
  const handleSubmit = useCallback(async (event) => {
    ...
    let bountyId = Date.now(); // Use timestamp as fallback ID
    let ledger = 0;
    ...
    try {
      ...
      try {
        const [bid, led] = await createBounty(
          walletAddress,
          title,
          Number(payAmount) * 10000000,
          DEF_PAY_TOKEN,
          SECS_PER_DAY * days
        );
        if (bid > 0) {
          bountyId = bid;
          ledger = led;
          toast.success('Successfully created on blockchain!');
        }
      } catch (e) {
        blockchainError = e;
        console.error('Blockchain error:', e);
        toast.warning(`Blockchain error: ${e.message || 'Insufficient funds or network error'}`);
      }

      // Try backend 
      const backendResult = await createBountyB(
        walletAddress,
        bountyId,
        title,
        Number(payAmount),
      ...
      if (!backendResult.success) {
        ...
        toast.warning(`Backend error: ${backendResult.error}`);
      } else {
        toast.success('Successfully saved to backend!');
      }

      // Continue if at least one succeeded
      if (blockchainError && !backendResult.success) {
        toast.error('Failed to create bounty anywhere');
        return;
      }
    } catch (e) {
    ...
    // Always show success and navigate
    toast.success('Successfully created bounty!');
    nav('/NewBounty/');
  }, [isConnected, walletAddress, title, payAmount, duration, type, difficulty, topic, description, gitHub]);
