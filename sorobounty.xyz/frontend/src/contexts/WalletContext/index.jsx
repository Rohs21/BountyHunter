...
export const WalletProvider = (props) => {
    ...
    const configuredChainId = useMemo(() => {
        ...
        return cfg ? parseInt(cfg.chainId, 16) : 0;
    }, [chainId]);
    ...
    const getSelectedWallet = () => {
        const v = window.localStorage.getItem('selectedWallet');
    ...
    const syncWallet = async (selectedWallet) => {
        ...
        if (selectedWallet === WalletType.WALLET_CONNECT) {
            ...
            try {
                ...
                console.error('syncWallet: failed to set wallet', e);
                return;
            }
        }
    ...
    const connectWallet = async () => {
        await kit.openModal({
            onWalletSelected: async (option) => {
                if (option && option.type) syncWallet(option.type);
            },
        });
    }
