...
export async function invoke<R extends ResponseTypes, T = string>({
  ...
  // Patch the server instance to normalize certain Soroban RPC payloads
  // (convert positional params for getLedgerEntries into named params)
  try {
    const anyServer: any = server as any;
    if (!anyServer.__patchedForParams) {
      // patch low-level _send if present
      if (typeof anyServer._send === "function") {
        const origSend = anyServer._send.bind(anyServer);
        anyServer._send = async function (path: any, body: any) {
          try {
            if (typeof body === "string") {
              const parsed = JSON.parse(body);
              if (
                parsed &&
                parsed.method === "getLedgerEntries" &&
                Array.isArray(parsed.params) &&
                parsed.params.length === 1 &&
                Array.isArray(parsed.params[0])
              ) {
      ...
      // patch fetch if present (some versions may use fetch)
      if (typeof anyServer.fetch === "function") {
        const origFetch = anyServer.fetch.bind(anyServer);
        anyServer.fetch = async function (input: any, init: any) {
          try {
